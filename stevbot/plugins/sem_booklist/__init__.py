    ...
    if args:
        ...
        if len(arg_list[1]) == 32:
            ...
            if M2EID.get(arg_list[1]) is not None:
                ...
                await booking_main.reject(f'{arg_list[1]}编号还未加入机器人的内置id列表！')
        i = 2 if len(arg_list) > 2 else 1
        result = await process_date(arg_list[i])
        if result:
            state['mm'], state['dd'] = result
        else:
            await booking_main.reject('日期格式不对或月份日数不正确，请确认是否是mmdd格式（如0809）')
    else:
        await booking_main.reject(r'''- /设备时间 [设备id] 日期
